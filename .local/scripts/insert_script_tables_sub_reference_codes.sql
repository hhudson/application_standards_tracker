...
spool &PROJECT_DIR/data/tables3/&FILE_NAME2
select '--liquibase formatted sql' lbstmt
from dual
union all
select '--changeset data_script:&FILE_NAME2 stripComments:false ' lbstmt
from dual 
union all
select '--preconditions onFail:MARK_RAN onError:HALT' lbstmt
from dual 
union all
select '--precondition-sql-check expectedResult:0 select count(1) from AST_SUB_REFERENCE_CODES asrc inner join EBA_STDS_STANDARD_TESTS esst on asrc.test_id = esst.id and esst.nt_type_id = &NT_TYPE_ID;' lbstmt
from dual
union all
...
select asrc.*  
from   except_cols (  
  AST_SUB_REFERENCE_CODES,  
  columns ( id, created, created_by, updated, updated_by )  
) asrc
inner join EBA_STDS_STANDARD_TESTS esst on asrc.test_id = esst.id and esst.nt_type_id =&NT_TYPE_ID
order by sub_code;
