...
select svt_deployment.json_content_clob (p_table_name => 'EBA_STDS_STANDARD_TESTS', p_test_code => '%0', p_standard_id => %2) thejsonclob from dual;]', vess.test_code, eba_stds.file_name(vess.calling_standard_name), ess.id, vess.file_name) stmt
...
select svt_deployment.json_standard_tests_clob (p_standard_id => '%0') thejsonclob from dual;]', standard_id, eba_stds.file_name(full_standard_name) ) stmt
