...
--liquibase formatted sql
--changeset table_script:SVT_PLSQL_APEX_AUDIT.COMPONENT_ID stripComments:false runOnChange:true
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tab_cols where table_name = 'SVT_PLSQL_APEX_AUDIT' and column_name = 'COMPONENT_ID';

alter table SVT_PLSQL_APEX_AUDIT add COMPONENT_ID number;
/
--liquibase formatted sql
--changeset table_script:SVT_PLSQL_APEX_AUDIT.PARENT_COMPONENT_ID stripComments:false runOnChange:true
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tab_cols where table_name = 'SVT_PLSQL_APEX_AUDIT' and column_name = 'PARENT_COMPONENT_ID';

alter table SVT_PLSQL_APEX_AUDIT add PARENT_COMPONENT_ID number;
/

CREATE INDEX SVT_PLAPAD_IDX2 
ON SVT_plsql_apex_audit(STANDARD_CODE)
/
CREATE INDEX SVT_PLAPAD_IDX3
ON SVT_plsql_apex_audit(application_id)
/