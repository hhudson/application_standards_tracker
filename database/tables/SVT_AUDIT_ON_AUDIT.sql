...
--liquibase formatted sql
--changeset table_script:SVT_AUDIT_ON_AUDIT.STANDARD_CODE stripComments:false runOnChange:true
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tab_cols where table_name = 'SVT_AUDIT_ON_AUDIT' and column_name = 'STANDARD_CODE';

alter table SVT_AUDIT_ON_AUDIT add standard_code varchar2(100)
/

--liquibase formatted sql
--changeset table_script:SVT_AUDIT_ON_AUDIT.AUDIT_ID stripComments:false runOnChange:true
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tab_cols where table_name = 'SVT_AUDIT_ON_AUDIT' and column_name = 'AUDIT_ID';

alter table SVT_AUDIT_ON_AUDIT add AUDIT_ID number
/

--liquibase formatted sql
--changeset table_script:SVT_AUDIT_ON_AUDIT.VALIDATION_FAILURE_MESSAGE stripComments:false runOnChange:true
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(1) from all_tab_cols where table_name = 'SVT_AUDIT_ON_AUDIT' and column_name = 'VALIDATION_FAILURE_MESSAGE';

alter table SVT_AUDIT_ON_AUDIT add VALIDATION_FAILURE_MESSAGE VARCHAR2(4000 CHAR)
/