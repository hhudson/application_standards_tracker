...
--changeset view_script:V_AST_PLSQL_APEX_AUDIT stripComments:false endDelimiter:/ runOnChange:true
--------------------------------------------------------
--  DDL for View V_AST_PLSQL_APEX_AUDIT
--------------------------------------------------------
...
-- 
    ...
    a.ast_component_type_id
from aspaa a
left outer join apex_issues ai on a.apex_issue_id = ai.issue_id
left outer join ast_sert_how_to_fix ahtf on ahtf.collection_name = a.standard_code
where a.include_in_report_yn = 'Y'
/
--rollback drop view V_AST_PLSQL_APEX_AUDIT;