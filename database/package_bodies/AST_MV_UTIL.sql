...
--changeset package_body_script:AST_MV_UTIL_body stripComments:false endDelimiter:/ runOnChange:true

create or replace package body AST_MV_UTIL as
----------------------------------------------------------------------------
...
---------------------------------------------------------------------------- 
  ...
  as
    c_scope          constant varchar2(128) := gc_scope_prefix || 'mv_ast_query';
    ...
    loop
      ...
      begin
        with thecols as (select column_name, column_id
                          from all_tab_cols
                          where table_name = 'MV_AST_BUTTONS'
                          union all
  ...
  procedure refresh_mv(p_mv_list in eba_stds_standard_tests.mv_dependency%type default null)
    ...
    begin
      ...
      if p_mv_list like '%V_MV_AST%' then
        for rec in (select object_name mview
                      from user_objects
                      where object_type = 'MATERIALIZED VIEW'
                      and object_name like 'MV_AST%'
                      order by 1)
        loop
...
/
--rollback drop package body AST_MV_UTIL;