...
--changeset package_body_script:AST_PREFERENCES_body stripComments:false endDelimiter:/ runOnChange:true

create or replace package body AST_PREFERENCES as
----------------------------------------------------------------------------
...
---------------------------------------------------------------------------- 
  ...
  begin
    ...
    and user_name = gc_ast
    ;

    return l_pref_value;
  exception 
    when no_data_found then 
      return case when p_preference_name = 'AST_DEFAULT_SCHEMA'
                  then 'AST'
                  else null
                  end;
    when others then
...
/
--rollback drop package body AST_PREFERENCES;