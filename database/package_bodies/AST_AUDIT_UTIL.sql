...
--changeset package_body_script:AST_AUDIT_UTIL stripComments:false endDelimiter:/ runOnChange:true
create or replace package body AST_AUDIT_UTIL as
----------------------------------------------------------------------------
...
---------------------------------------------------------------------------- 
  ...
  is
  c_scope constant varchar2(128) := gc_scope_prefix || 'v_ast_scm_object_assignee';
  ...
  is
      ...
      $then 
        with scm as (select case when cfo.folder_name = 'Bodies'
                    ...
                    cfi.checked_out_by,
                    case when cfi.created_by not in ('UT_CARS', 'CARS', 'APEX_SCM', ast_preferences.get_preference ('AST_DEFAULT_SCHEMA'))
                         then cfi.created_by
                         end created_by, 
                    case when cfi.updated_by not in ('UT_CARS', 'CARS', 'APEX_SCM', ast_preferences.get_preference ('AST_DEFAULT_SCHEMA'))
                         then cfi.updated_by
                         end updated_by
  ...
  begin
    ...
    loop
      ...
      loop
        pipe row (v_ast_scm_object_assignee_ot (
  ...
  procedure recompile_w_plscope is 
    ...
    begin
      ...
      delete from ast_plsql_apex_audit
      where 1=1
      and owner = case when sys_context('userenv', 'current_user') = 'AST'
    ...
    ------------------------------------------------------------------------------
    -- Procedure to merge data into ast_plsql_apex_audit
    ------------------------------------------------------------------------------
    ...
    type r_issues is record (
        ...
        pass_yn                    ast_plsql_apex_audit.pass_yn%type,
        line                       ast_plsql_apex_audit.line%type,
        object_name                ast_plsql_apex_audit.object_name%type,
        object_type                ast_plsql_apex_audit.object_type%type,
        code                       ast_plsql_apex_audit.code%type,
        validation_failure_message ast_plsql_apex_audit.validation_failure_message%type,
        issue_title                ast_plsql_apex_audit.issue_title%type,
        standard_code              ast_plsql_apex_audit.standard_code%type,
        child_code                 ast_plsql_apex_audit.child_code%type,
        apex_created_by            ast_plsql_apex_audit.apex_created_by%type,
        apex_created_on            ast_plsql_apex_audit.apex_created_on%type,
        apex_last_updated_by       ast_plsql_apex_audit.apex_last_updated_by%type,
        apex_last_updated_on       ast_plsql_apex_audit.apex_last_updated_on%type,
        owner                      ast_plsql_apex_audit.owner%type,
        component_id               ast_plsql_apex_audit.component_id%type,
        parent_component_id        ast_plsql_apex_audit.parent_component_id%type
    );
    type t_issue_rec is table of r_issues index by ast_plsql_apex_audit.unqid%type; 
    l_issues_t            t_issue_rec;
    type t_unqid_rec is table of ast_plsql_apex_audit.unqid%type; 
    l_current_issues_t    t_unqid_rec := t_unqid_rec();
    c_legacy_yn  constant ast_plsql_apex_audit.legacy_yn%type := case when upper(p_legacy_yn) = 'Y'
                                                                      then 'Y'
                                                                      else 'N'
                                                                      end;
    ...
    begin
      apex_debug.message(c_debug_template,'START',
        ...
        for rec in (select 
                    ...
                    ast_ctx_util.get_default_user owner,
                    a.component_id,
                  ...
                  join ast_standard_view.v_ast(p_standard_code => esst.standard_code, 
                                               ...
                                               p_unqid => l_ast_plsql_apex_audit_rec.unqid
                                               ) a
        ...
        begin
          ...
          loop
            ...
            using (select l_issues_t(l_unqid).issue_category             issue_category,
                          ...
                          l_issues_t(l_unqid).child_code                 child_code,
    ...
    c_workspace constant apex_workspaces.workspace%type := case when p_workspace is not null 
                                                                ...
                                                                when ast_preferences.get_preference ('AST_DEFAULT_WORKSPACE') is not null 
...
------------------------------------------------------------------------------
    ...
    type t_object_assignee is table of ast_plsql_apex_audit.assignee%type index by ast_plsql_apex_audit.object_name%type;
    l_object_assignee t_object_assignee;
    l_object ast_plsql_apex_audit.object_name%type;
    begin
      apex_debug.message(c_debug_template,'START');

      for rec in (select object_name, email assignee 
                  from v_ast_scm_object_assignee
...
------------------------------------------------------------------------------
    ...
    type t_auditid_email is table of ast_plsql_apex_audit.assignee%type index by pls_integer;
    l_auditid_email t_auditid_email;
    l_auditid ast_plsql_apex_audit.id%type;
    begin
      apex_debug.message(c_debug_template,'START');
...
------------------------------------------------------------------------------
    ...
    begin
      ...
      if ast_preferences.get_preference ('AST_DEFAULT_ASSIGNEE') is not null then 
        update ast_plsql_apex_audit
        set assignee = ast_preferences.get_preference ('AST_DEFAULT_ASSIGNEE')
    ...
    procedure record_daily_issue_snapshot(p_application_id in ast_plsql_apex_audit.application_id%type default null,
     ...
     begin
        ...
        ast_ctx_util.set_review_schema;
