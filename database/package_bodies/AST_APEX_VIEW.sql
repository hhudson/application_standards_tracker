...
--changeset package_body_script:AST_APEX_VIEW stripComments:false endDelimiter:/ runOnChange:true
create or replace package body ast_apex_view as
----------------------------------------------------------------------------
...
---------------------------------------------------------------------------- 
  ...
  as
    ...
    c_user constant all_users.username%type := coalesce(p_user, 
                                                        case when c_userenv_current_user = 'AST'
  ...
  begin
    ...
    loop
      ...
      loop
        pipe row (ast_apex_applications_ot (
  ...
  cursor cur_awp
    is select 
       ...
       $then 
        ...
        where preference_name like 'AST%'
        -- and user_name = 'AST' --doesn't work with AST_EMAIL_SUBSCRIPTION
       $else 
        null workspace_id,
        null workspace_name,
        ...
        'HAYHUDSO' user_name,
        null preference_id,
        'AST_EMAIL_SUBSCRIPTION' preference_name,
  ...
  begin
    ...
    loop
      ...
      loop
        pipe row (ast_apex_preferences_ot (
