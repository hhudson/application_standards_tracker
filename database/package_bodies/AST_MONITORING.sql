...
--changeset package_body_script:AST_MONITORING stripComments:false endDelimiter:/ runOnChange:true
create or replace package body AST_MONITORING as
----------------------------------------------------------------------------
...
---------------------------------------------------------------------------- 
  ...
  gc_ast_app_id   constant apex_applications.application_id%type := 17000033;
  ...
  cursor cur_ast (p_standard_code varchar2) is 
  select  case when length(pad.issue_title) > gc_max_title_length 
         ...
         pad.audit_id
  from v_ast_plsql_apex_audit pad
  ...
  begin
    ...
    return case 
           when ast_preferences.get_preference (p_preference_name => 'AST_DB_NAME') is not null 
           then ast_preferences.get_preference (p_preference_name => 'AST_DB_NAME')
...
------------------------------------------------------------------------------
  ...
  begin
    ...
    begin <<subs_lst>>
      ...
      from v_ast_email_subscriptions;
    ...
    apex_mail.prepare_template (
      ...
      p_application_id    => gc_ast_app_id, 
      p_subject           => p_subject,
      p_html              => l_html,
      p_text              => p_text
    );
  ...
  begin
    ...
    if ast_preferences.get_preference ('AST_EMAIL_API') = gc_apex then
      <<apx_ml>>
      declare 
      l_id number;
      begin
  ...
  c_from_email constant varchar2(50) := ast_preferences.get_preference ('AST_FROM_EMAIL');
...
--rollback drop package AST_MONITORING;
